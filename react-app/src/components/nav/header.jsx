...
export default function Nav({onTabChanged,tabs = [], title, historyEnabled, defaultTabIndex =0,  }){
    ...
    return (
<div className="fixed top-0 left-0 h-20 w-full bg-gold text-white z-[100] flex justify-center">
    <div className="max-w-screen-md w-full mx-auto relative flex items-center justify-center">
        
        <img src="/AUMBC_logo.png" alt="logo" className="absolute top-1 right-4 w-12" />
        
        {historyEnabled && (
            <button className="absolute top-3 left-4" onClick={() => navigate(-1)}>
                <Icon icon="material-symbols:keyboard-arrow-left" className="w-8 h-8" />
            </button>
        )}

        <div className="absolute top-3.5 font-bold">{title}</div>
        ...
        {tabs.length > 0 && (
            <div className="absolute left-4 top-12 w-[calc(100%-2rem)] h-7 flex items-center bg-gold-dark rounded-full">
                <div
                    ...
                    style={{
                        width: tabs.length === 3 ? "calc(33.33%)" : "calc(50%)",
